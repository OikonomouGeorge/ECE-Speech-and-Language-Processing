...
def train_dataset(_epoch, dataloader, model, loss_function, optimizer, DATASET):
    ...
    for index, batch in enumerate(dataloader, 1):
        ...
        # EX9
        if model.__class__.__name__ in ["SimpleSelfAttentionModel", "MultiHeadAttentionModel", "TransformerEncoderModel"]:
    ...
    with torch.no_grad():
        for index, batch in enumerate(dataloader, 1):
            ...
            if outputs.dim() == 2 and outputs.shape[1] == 1:
                y_preds = (outputs > 0).long().squeeze(1)
            else:
                y_preds = torch.argmax(outputs, dim=1)

            # Step 5 - collect the predictions, gold labels and batch loss
            y_pred.append(y_preds) # EX9
            y.append(labels)  # EX9
            running_loss += loss.data.item()
